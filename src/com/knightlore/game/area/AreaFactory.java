...
public final class AreaFactory {
    ...
    public static Map createRandomMap(IEnvironment environment) {
        ...
        /*
         * int r = 0; // make horizontal rows of free space for (int i = 0; i < TODO dead code? delete?
    ...
    private static void placeSubArea(Tile[][] grid, int x, int y, Area subArea) {
        if (x + subArea.width >= grid.length || y + subArea.height >= grid[0].length) {
            throw new IndexOutOfBoundsException("One of the sub areas during" +
                    " map generation was partially generated outside of the map");
        }
