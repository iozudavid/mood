...
public class Shotgun extends Weapon {
    ...
    public Shotgun() {
        super(WeaponSprite.SHOTGUN, false,
                (int) (GameEngine.UPDATES_PER_SECOND * 0.25D), SHOOT_SFX);
    }
    ...
    public void fire(Entity shooter) {
        ...
        RaycastHit hit = GameEngine.getSingleton().getWorld().raycast(
                shooter.getPosition(), shooter.getDirection(), 100, sqrRange,
                shooter);
        if (hit.didHitEntity()) {
            // take damage!
            hit.getEntity().takeDamage(
                    damageInflicted(shooter, hit.getEntity()), shooter);
        }
    }
    ...
    public int damageInflicted(Entity shooter, Entity target) {
        ...
        Vector2D displacement = shooter.getPosition()
